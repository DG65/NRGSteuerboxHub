...
|---|---|---|---|
| EMS | 0.22.0 (20.08.2026: Tagesplan-Umbau + `EMS_GetDayPlan()` fuer Dashboard-Visualisierung, **noch nicht mehrtägig live verifiziert**, siehe EMS/CLAUDE.md) | ems-integration | konsumiert alle; künftig `EMS_GetSpecialEvents`. Steuerlogik jetzt vorausschauend: `BuildDayPlan()` plant alle 96 Tages-Viertelstunden aus PT15M-Preisen (automatisch via `TIBBERGR_GetPriceCurve`, Fallback manuell)+PVF+Lastschätzung, sichtbar als Symcon-Wochenplan unter der Instanz (Vorbild: Dietmars Winterskript #55729), `optimize()` fragt nur noch den Plan ab statt live gegen Schwellwerte zu prüfen. Alte `SetECOWindow()`-Planer (GoodWe-ECO-Register, liefen unabhängig vom laufenden `optimize()`) entfernt. Neu: `EMS_GetDayPlan()` **1.0** liefert heute+morgen (Zeit/Op/Preis/simulierter SOC je Slot) fuer externe Visualisierung — der native Kalender bleibt auf "heute" begrenzt (Kalender-Typ-Grenze). |
